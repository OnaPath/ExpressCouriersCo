if (!window.DeliveryFormHandler) {
    ...
    if (!window.CITY_BOUNDS) {
        window.CITY_BOUNDS = {
            ...
            'airdrie': { north: 51.3, south: 51.2, east: -113.9, west: -114.1 },
            'lethbridge': { north: 49.7, south: 49.6, east: -112.7, west: -112.9 }
        };
    }
    ...
    class DeliveryFormHandler {
        ...
        async initializeGoogleMaps() {
            try {
                ...
                if (!this.mapsApiKey) throw new Error('Maps API key not available');
        ...
        async initializeMonerisScript() {
            try {
                if (typeof MonerisCheckout !== 'undefined') {
                    console.log('Moneris script already loaded');
                    return;
                }
                ...
                await new Promise((resolve, reject) => {
                    ...
                    script.onerror = (error) => {
                        console.error('Moneris script failed to load:', error);
                        reject(error);
                    };
        ...
        setupAddressAutocomplete() {
            ...
            addressInputs.forEach(input => {
                ...
                autocomplete.addListener('place_changed', () => {
                    ...
                    if (lat < cityBounds.south || lat > cityBounds.north || lng < cityBounds.west || lng > cityBounds.east) {
        ...
        initializePayment() {
            const cityConfigs = {
                calgary: { city: 'calgary', deliveryFee: 21.00, distanceSurcharge: 5.00, rushHourFee: 2.50, gstRate: 0.05 },
                airdrie: { city: 'airdrie', deliveryFee: 21.00, distanceSurcharge: 0, rushHourFee: 0, gstRate: 0.05 },
                lethbridge: { city: 'lethbridge', deliveryFee: 20.00, distanceSurcharge: 0, rushHourFee: 0, gstRate: 0.05 }
            };

            const cityInput = this.form.querySelector('input[name="city"]');
            this.city = cityInput ? cityInput.value : 'calgary';
            this.paymentConfig = cityConfigs[this.city] || { city: this.city, deliveryFee: 20.00, distanceSurcharge: 0, rushHourFee: 0, gstRate: 0.05 };
        ...
        showMonerisIframe() {
            if (typeof MonerisCheckout === 'undefined') {
                console.error('MonerisCheckout not available—script failed to initialize');
